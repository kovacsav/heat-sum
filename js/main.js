...
const validateDate = () => {
  if (
    typeof start.getRange().start == "undefined" ||
    !(start.getRange().start.constructor.toString().indexOf("Date") > -1)
  ) {
    alert("Kérem válasszon kezdő időpontot!");
    return false;
  } else {
    if (
      typeof start.getRange().end == "undefined" ||
      !(start.getRange().end.constructor.toString().indexOf("Date") > -1)
    ) {
...
const validateDateRange = () => {
  ...
  if (
    ...
    endingDate.getTime() - startingDate.getTime() > maxDateRange
  ) {
    alert(
      "A kiválasztott időszak túl hosszú, mert csak egy vegetációs időszakra számolunk hőösszeget. Kérem javítsa a kezdő vagy a záró időpontot!"
    );
    return false;
  } else {
    return true;
...
const validateTemperature = (base, top) => {
  if (
    ...
    alert(
      "A bázishőmérsékletnek 0 és 20, a maximális hőmérsékletnek pedig 0 és 40 Celsius fok közé kell esnie."
    );
    return false;
  } else {
    return true;
...
const validateInputData = () => {
  if (validateDate()) {
    // a dátumok validálása
    //console.log("Dates are OK");

    if (validateDateRange()) {
      // az időszak hossza nem lehet több, mint egy vegetációs időszak
...
const createTemperatureTimeSeries = (dateRangeObject) => {
  ...
  const days =
    (dateRangeObject.end - dateRangeObject.start) / (24 * 60 * 60 * 1000) + 1; // millisecundumban számolja és +1 mert a végidőpont is beleszámít
...
const effectiveTemperatureCalculator = (
  dateRangeObject,
  minTemperature,
  maxTemperature,
  cummulatedArray
  ...
  cummulatedArray[0] = {
    ...
    y:
      Math.round(
        dailyEffectiveTemperature(minTemperature[0], maxTemperature[0]) * 10
      ) / 10,
  };
  ...
  for (let i = 1; i < minTemperature.length; i++) {
    ...
    cummulatedArray.push({
      ...
      y:
        Math.round(
          (cummulatedArray[i - 1].y +
            dailyEffectiveTemperature(minTemperature[i], maxTemperature[i])) *
            10
        ) / 10,
    });
  }
...
const getCummulatedEffectiveTemperature = () => {
  ...
  effectiveTemperatureCalculator(
    ...
    averageCummulatedEffectiveTemperature
  );
  ...
  effectiveTemperatureCalculator(
    ...
    minTemperature,
    maxTemperature,
    cummulatedEffectiveTemperature
  );
};
      ...
      datasets: [
        {
          ...
          fill: {
            // ez a lehetőség csak a 3.x verziótól elérhető
...
selectionButton.addEventListener("click", (ev) => {
  ...
  if (validateInputData()) {
    // ha minden bemenő adat OK, akkor
